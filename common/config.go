...
type RunnerSettings struct {
	...
	SetUID          bool     `toml:"setuid,omitempty" json:"setuid" long:"setuid" env:"RUNNER_SETUID" description:"Set the UID of the executor to the logged-in user in the Gitlab Web UI."`
	SetUIDDataDir   string   `toml:"setuid_data_dir,omitempty" json:"setuid_data_dir" long:"setuid-data-dir" env:"RUNNER_SETUID_DATA_DIR" description:"Directory where SetUID Runner builds and cache are stored. If set to $HOME, each will be stored in the given users home directory, otherwise each SetUID Runner will have it's own user-specific workspace below this directory in the form of /users/<user>"`
	SetUIDUserWhitelist []string `toml:"setuid_user_whitelist,omitempty" json:"setuid_user_whitelist" long:"setuid-user-whitelist" env:"RUNNER_SETUID_USER_WHITELIST" description:"Used only if SetUID is enabled (true). An authoratative list of users who can execute Gitlab SetUID Runners."`
  SetUIDUserBlacklist []string `toml:"setuid_user_blacklist,omitempty" json:"setuid_user_blacklist" long:"setuid-user-blacklist" env:"RUNNER_SETUID_USER_BLACKLIST" description:"Used only if SetUID is enabled (true). A list of usernames that are not allowed to run CI jobs. More authoritative than group whitelist / blacklist, but can be overridden by SetUIDUserWhitelist."`
	SetUIDGroupWhitelist []string `toml:"setuid_groups_whitelist,omitempty" json:"setuid_groups_whitelist" long:"setuid-groups-whitelist" env:"RUNNER_SETUID_GROUP_WHITELIST" description:"Use only if SetUID is enabled. A list of groups that are allowed to run CI jobs. Least authoritative"`
	SetUIDGroupBlacklist []string `toml:"setuid_groups_blacklist,omitempty" json:"setuid_groups_blacklist" long:"setuid-groups-blacklist" env:"RUNNER_SETUID_GROUP_BLACKLIST" description:"Use only if SetUID is enabled. A list of groups that are not allowed to run CI jobs."`
